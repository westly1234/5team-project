...
def get_ai_keywords(request):
    ...
    이 상황에 어울리는 유튜브 '음악 플레이리스트' 또는 '음악 믹스'를 찾을 수 있는 검색 키워드 2개를 추천해주세요.
    ...
    **규칙:**
    ...
    - 출력은 줄바꿈(\n)으로 구분된 키워드 2개만 반환해야 하며, 번호나 설명, 따옴표는 절대 포함하지 마세요.
    {personalization_prompt_part}
    """
...
def handle_music_preference(request):
    """사용자의 음악 선호도(보관하기)를 처리하고 업적을 확인합니다."""
    try:
        data = json.loads(request.body)
        video_id = data.get('videoId')
        video_title = data.get('videoTitle')
        pref_type = data.get('preferenceType')

        if not all([video_id, video_title, pref_type]):
            return JsonResponse({'error': '필수 데이터가 누락되었습니다.'}, status=400)

        # 기존 기록이 있으면 업데이트, 없으면 생성 (주로 생성될 것)
        obj, created = UserMusicPreference.objects.update_or_create(
            user=request.user,
            video_id=video_id,
            preference_type=pref_type, # 'saved'로 고정될 것
            defaults={'video_title': video_title}
        )
        
        # '내 보관함 지킴이' 업적: 첫 음악 보관 시 (새로운 업적 ID 가정)
        if pref_type == 'saved' and created:
